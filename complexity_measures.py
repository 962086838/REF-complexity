...
def _pacbayes_sigma(
        ...
        accuracy_displacement: float = 0.05,
    ...
    for _ in range(search_depth):
        ...
        for _ in range(montecarlo_samples):
            with _perturbed_model(model, sigma, rng, magnitude_eps) as p_model:
                ...
                for data, target in dataloader:
                    ...
                    logits = p_model(data)
                    # print('logits', logits, logits.shape) # batchsize, 100
                    pred = logits.data.max(1, keepdim=True)[1]  # get the index of the max logits
                    # print('pred', pred, pred.shape)  # batchsize, 1
        ...
        print('sigma', sigma, "np.mean(accuracy_samples)", np.mean(accuracy_samples), "accuracy", accuracy)
...
def get_all_measures(model, init_model, dataloader,
    ...
    print("INVERSE_MARGIN dist complete")
    ...
    measures[CT.LOG_SUM_OF_SPEC_OVER_MARGIN] = math.log(d) + (1 / d) * (
                measures[CT.LOG_PROD_OF_SPEC] - 2 * margin.log())  # 34
    measures[CT.LOG_SUM_OF_SPEC] = math.log(d) + (1 / d) * measures[CT.LOG_PROD_OF_SPEC]  # 35
    print("LOG_SUM_OF_SPEC complete")
    ...
    measures[CT.LOG_SUM_OF_FRO_OVER_MARGIN] = math.log(d) + (1 / d) * (
                measures[CT.LOG_PROD_OF_FRO] - 2 * margin.log())  # 38
    measures[CT.LOG_SUM_OF_FRO] = math.log(d) + (1 / d) * measures[CT.LOG_PROD_OF_FRO]  # 39
    print("LOG_SUM_OF_FRO complete")
    ...
    def _path_norm(model) -> Tensor:
        ...
        for param in model.parameters():
            if param.requires_grad:
                param.data.pow_(2)
        x = torch.ones([1] + list(dataset_type_D),
                       device=device)  # modified line, replaced model.dataset_type.D with dataset_type_D
        x = model(x)
        del model
        return x.sum()
    ...
    print("PATH_NORM_OVER_MARGIN complete")
    ...
    print("PACBAYES_FLATNESS complete")
    ...
    print("PACBAYES_MAG_FLATNESS complete")
